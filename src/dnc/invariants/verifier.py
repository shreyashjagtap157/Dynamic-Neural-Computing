...
class BootstrapVerifier:
    ...
    def _check_inv2_dag(self, es: Any) -> VerificationResult:
        ...
        A graph with edges that form a cycle is invalid.
        """
        try:
            graph = getattr(es, "G", None)
            if graph is None:
                return VerificationResult("INV-2", VerificationStatus.SKIP, "No execution graph set")

            if hasattr(graph, "_in_degree") and not graph._in_degree:
                return VerificationResult("INV-2", VerificationStatus.SKIP, "Empty graph — no nodes to verify")
