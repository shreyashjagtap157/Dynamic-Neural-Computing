...
| INTENT-006 | Use calibrated risk control rather than raw model confidence | RFC evidence/confidence rules; `src/dnc/cognition/calibration.py`; `src/dnc/cognition/controller.py` | Calibration registry and halting policy | Held-out risk-coverage evidence |
| INTENT-007 | Detect semantic saturation and answer novelty | Master plan Phase 5 | Semantic cluster records | Attempts after halt add no material supported claim under test |
