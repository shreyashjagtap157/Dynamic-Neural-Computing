...
class TestINV5_HistoryLogAppendOnly:
    ...
    def test_inv5_append_returns_index(self):
        """Appending to history log returns a monotonically increasing index."""
        hl = HistoryLog()
        mid = ModuleInstanceID("Test")
        idx1 = hl.append("OUTPUT_BIND", mid, UNBOUND(), "value", "prov_1")
        idx2 = hl.append("OUTPUT_BIND", mid, "value", "value2", "prov_2")
        assert idx2 > idx1
        assert hl[idx1]["after_value"] == "value"
        assert hl[idx2]["after_value"] == "value2"
