...
class DistributedHandoffConfig:
    """Per INV-DIST-6: timeout and retry configuration for distributed handoffs.

    Defaults per DEF-DIST-3 and distributed-handoff-protocol.md Section 7.
    """
...
class DistributedHandoffRecord:
    ...
    Per INV-DIST-3: retry_count tracks consecutive failures for partition detection.
    Per INV-DIST-4: error field captures partition/failure reason for coordinator failover.
    """
...
class DistributedCoordinator:
    ...
    Per INV-DIST-3: tracks retry_count for partition detection, provides retry_with_backoff.
    Per INV-DIST-4: mark_failed records error for control loop recovery.
    Per INV-DIST-5: enforces single-hop (direct sender→receiver only).
    """
    ...
    def __init__(
        ...
        config: Optional[DistributedHandoffConfig] = None,
    ) -> None:
        self._node_id = node_id
        self._mode = mode
        self._config = config or DistributedHandoffConfig()
    ...
    @property
    def config(self) -> DistributedHandoffConfig:
        return self._config
    ...
    def initiate_handoff(
        ...
        Per INV-DIST-5: validates single-hop constraint (direct sender→receiver only).
        Multi-hop routing is not supported in Baseline v1.0.
        """
        if sender_node_id == receiver_node_id:
            raise ValueError(
                "INV-DIST-5 violation: inter-node handoffs must be direct sender->receiver. "
                "Multi-hop is not supported in Baseline v1.0."
            )
    ...
    def receive_message(
        ...
        if receiver.is_duplicate(message.message_id):
            receiver.get_cached_result(message.message_id)
    ...
    def retry_with_backoff(
        ...
        """Per INV-DIST-3: compute backoff delay given consecutive failure count.

        Levels:
        - 0-2: exponential (1s, 2s, 4s)
        - 3-5: exponential with jitter
        - 6+: fail-fast (return 0, caller should mark FAILED)

        Returns delay in seconds.
        """
        if consecutive_failures <= 2:
            base = 1.0 * (2 ** consecutive_failures)
            return base
    ...
    def mark_failed(
        ...
        """Per INV-DIST-4: mark handoff as FAILED with error for control loop recovery.

        After max_retries, the control loop is responsible for recovery.
        No automatic coordinator failover is performed in Baseline v1.0.
        """
        record = self._handoff_records.get(handoff_id)
        if record:
            record.status = HandoffStatus.FAILED
            record.error = reason
            record.retry_count = self._config.max_retries
