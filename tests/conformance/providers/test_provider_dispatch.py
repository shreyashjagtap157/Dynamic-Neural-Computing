...
class TestProviderDispatch:
    ...
    def test_prod_mode_routes_through_provider(self):
        ...
        runtime.set_node_capabilities(
            {
                mids["SRC"]: ExecutionCapability.CAP_EXECUTION,
                mids["TR"]: ExecutionCapability.CAP_REASONING,
                mids["SINK"]: ExecutionCapability.CAP_EXECUTION,
            }
        )
        ...
        assert runtime.state == ExecutionState2.TERMINATED
        # TR is served by the reasoning provider, so its output is tagged.
        tr_out = es.W[mids["TR"]].output
        assert isinstance(tr_out, str) and tr_out.startswith("[Reference:CAP_REASONING]"), (
