...
def render(invariants, cov, conf_pass, conf_total, conf_rows, acd_res, acd_tot, spec_ok) -> str:
    ...
    lines.append("")
    lines.append("- `LinearGraphPlanner` synthesizes linear graphs only (single synthesis pass).")
    lines.append("- No learned DecisionPolicy (RulePolicy is the reference policy).")
    lines.append("- Multi-hop distributed handoffs deferred to v1.1 (only direct sender -> receiver).")
    lines.append("- Internet-dependent providers (OpenAI/Anthropic/Gemini) require API keys and")
    lines.append("  network access; offline tests use the reference provider path.")
    lines.append("")
