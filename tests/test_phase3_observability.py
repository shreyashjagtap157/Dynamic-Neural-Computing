...
class TestPhase3ExitCriteria:
    ...
    def test_ec3_hash_chain_integrity(self):
        ...
        tampered_event = ProvenanceEvent(
            event_id=e1.event_id,
            event_type=e1.event_type,
            timestamp=e1.timestamp,
            execution_id=e1.execution_id,
            step_index=e1.step_index,
            causal_ref=e1.causal_ref,
            payload={"tampered": True},
            integrity_hash=e1.integrity_hash,
        )
        pl._events[1] = tampered_event
        pl._event_index[e1.event_id] = tampered_event
    ...
    def test_ec5_failure_rate_tracking(self):
        ...
        classifier = FailureClassifier()
        classifier.record_failure("transformer_v1", total_invocations=50)
        classifier.record_failure("transformer_v1", total_invocations=50)
    ...
    def test_ec6_evaluation_trial_count(self):
        ...
        dnc_values = [0.72, 0.78, 0.81, 0.75, 0.83, 0.77, 0.82, 0.79,
                     0.85, 0.74, 0.80, 0.76, 0.84, 0.78, 0.82, 0.73,
                     0.79, 0.86, 0.75, 0.81, 0.77, 0.83, 0.79, 0.74,
                     0.88, 0.76, 0.82, 0.78, 0.80, 0.75]
        baseline_values = [0.60, 0.62, 0.58, 0.61, 0.59, 0.63, 0.60, 0.62,
                           0.57, 0.61, 0.59, 0.63, 0.58, 0.60, 0.62, 0.59,
                           0.61, 0.57, 0.63, 0.60, 0.62, 0.58, 0.61, 0.59,
                           0.63, 0.60, 0.62, 0.57, 0.61, 0.59]

        assert len(dnc_values) == 30, f"DNC list has {len(dnc_values)}"
        assert len(baseline_values) == 30, f"Baseline list has {len(baseline_values)}"
    ...
    def test_ec9_drift_bound_rejected(self):
        ...
        is_within_far, far_violations = dr.is_within_drift_bound([0.2, 0.2, 0.2], DRIFT_BOUND)
        assert is_within_far is False, "Far vector should exceed drift bound"
