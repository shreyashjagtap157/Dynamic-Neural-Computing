...
**Specification Version**: 0.1.0 (Baseline v1.0; Architecture v1.0 frozen)
**Implementation Version**: 0.1.0 (Phase 1–7 complete, 154/154 tests passing)
**Reference Runtime**: Baseline v1.1 — conformant with Architecture v1.0 (ACD-001..ACD-004 resolved; conformance report at `docs/conformance-report.md`)
...
**Architecture Conformance Review**: PASSED — reference runtime conforms to Architecture v1.0 (Baseline v1.1 milestone, ARCH-AMEND-002).

> **Architecture v1.0 is frozen.** Future phases preserve it as the stable
> specification baseline; new planners, providers, metrics, and learning
> mechanisms are conforming **extensions**, not architectural revisions.
...
**generate_conformance_report.py** — Regenerates `docs/conformance-report.md`, the
Architecture Conformance Report release artifact:

```bash
# Regenerate the conformance report (runs the conformance suite + spec tooling)
python tools/generate_conformance_report.py

# CI gate: exit non-zero if the runtime is not fully conformant
python tools/generate_conformance_report.py --check
```
...
### Architecture Conformance Review (Baseline v1.1 milestone)

The Architecture Conformance Review is PASSED. Findings and remediation:

- **ACD-001..ACD-004 RESOLVED** (see the ACD table above); each closed by an
  executable conformance test in `tests/conformance/`.
- **Conformance suite**: `tests/conformance/{runtime,replay,providers}/*.py`
  (15 tests) drive the runtime, replay engine, providers, and rollback path.
- **Conformance report**: `docs/conformance-report.md` is a release artifact
  regenerated by `tools/generate_conformance_report.py` (reports ACD status,
  conformance pass counts, INV coverage per namespace, deviations, limitations).
- **Spec tooling**: `fix_refs.py --verify` (PR-4) and `--rfc2119` (PR-7) PASS.

> Architecture v1.0 is approved as conformant. Remaining work concerns
> capability expansion rather than architectural correction.

### Phase 6 — Adaptive Execution Research (next milestone, post-v1.1)

Phase 6 is redefined from "LLM integration" to **Adaptive Execution Research**.
It produces new *conforming implementations* under the frozen Architecture v1.0
(no architectural revisions):

- Arbitrary DAG synthesis (generalize `LinearGraphPlanner` beyond linear graphs)
- Learned DecisionPolicy (RL/LLM policy alongside the reference `RulePolicy`)
- Computation-aware evaluation benchmark + trace dataset
- Adaptive planning research + execution-policy learning
- `LinearGraphPlanner` alias retained; new planners implement the existing
  `Planner` interface as conforming extensions

### Active
- None — Baseline v1.1 tagged; Phase 6 (Adaptive Execution Research) is the next milestone
