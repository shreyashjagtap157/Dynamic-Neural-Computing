...
# h1 = sigmoid(10*x1 + 10*x2 - 5)        # ~1 when at least one input is 1 (OR)
# h2 = sigmoid(10*x1 + 10*x2 - 15)       # ~1 only when both inputs are 1 (AND)
...
class TestRealWorldNeuralCompute:
    ...
    def _run_sample(self, x1: float, x2: float) -> int:
        ...
        # Natural termination: the control loop dispatches runnable nodes, the
        # completion guard prevents re-execution, and once every node has
        # produced an output pending_count == 0 so decide() returns TERMINATE.
        for _ in range(20):
        ...
        assert not isinstance(buf, (UNBOUND, PENDING)), "DEC must be complete after termination"
        return int(buf)
