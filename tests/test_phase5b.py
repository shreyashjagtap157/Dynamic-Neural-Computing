...
def test_ec3_rule_policy_replan_when_resource_exhausted():
    """EC-3: RulePolicy returns REPLAN when resource budget is exhausted (if can_replan)."""
    policy = RulePolicy(resource_threshold=0.5)
    es = ExecutionState()
    es._budget_remaining = 0.1
    obs = {}

    decision = policy.decide(obs, es)
    assert decision == Decision.REPLAN, f"Expected REPLAN, got {decision}"
    print("PASS: ec3_rule_policy_replan_when_resource_exhausted")
...
if __name__ == "__main__":
    tests = [
        ...
        test_ec3_rule_policy_replan_when_resource_exhausted,
