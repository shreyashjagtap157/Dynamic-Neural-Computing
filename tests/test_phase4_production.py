...
class MockMetricResult:
    def __init__(self, metric_name: str, classification: ResultClassification) -> None:
...
def test_ec2_regression_monitor_detects_class1_regression():
    ...
    prior_results = {
        "scenario_1": {"accuracy": ResultClassification.WIN},
    }

    eval_run = EvaluationRun(
        run_id="run_1",
        scenario_id="scenario_1",
        runtime_under_test="dnc_v1",
        baseline_ids=["baseline_1"],
        metric_results=[
            MockMetricResult("accuracy", ResultClassification.LOSS),
        ],
        timestamp=0.0,
    ...
    regressions = suite.detect_regression(eval_run, prior_results)
    assert len(regressions) > 0, "Should detect Class 1 regression (WIN → LOSS)"
...
def test_ec3_bias_evaluation_framework_demographic_parity():
    ...
    assert within, f"Demographic parity diff {diff:.4f} should be within 0.05 (rates: A={rate_a:.4f}, B={rate_b:.4f})"
